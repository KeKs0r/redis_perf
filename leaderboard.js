...
/*
  Leaderboard Setup Stuff
*/
var Leaderboard = require('agoragames-leaderboard/lib/competition_ranking_leaderboard');

var boards = {}
const getLeaderboard = function getLeaderboard(key) {
  if(!boards[key]){
    boards[key] = new Leaderboard(key);
  }
  return boards[key];
};
...
function loadFixtures(userCount, cb) {
    ...
    Async.forEachOfLimit(matchdays, matchdayDayConcurrency, function (matchday, key, cb) {
        ...
        var leaderboard = getLeaderboard(key);
        if (matchday.length > realChunkSize) {
            ...
            Async.each(chunks, function (chunk, next) {
                ...
                leaderboard.rankMembers(chunk, function(res) {
            ...
            console.time('    ' + key);
            leaderboard.rankMembers(matchday, function(res){
              console.timeEnd('    ' + key);
              cb(err, res);
            });
        }
